                        ...
                        <td>
                            <a href="{{ route('post.show', $post->id) }}">
                                <img src="{{ asset('/storage/images/' . $post->image) }}" alt="{{ $post->image }}"
                                    class="d-block mx-auto img-fluid" style="width: 7rem; height: 7rem;  object-fit: cover;">
                            </a>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu">
                                    @if ($post->trashed())
                                        <button type="button" class="dropdown-item text-primary btn btn-sm shadow-none" data-bs-toggle="modal"
                                            data-bs-target="#show-post-{{ $post->id }}">
                                            <i class="fa-solid fa-eye"></i> Show Post {{ $post->id }}
                                        </button>
                                    @else
                                        <button type="button" class="dropdown-item text-danger btn btn-sm shadow-none" data-bs-toggle="modal"
